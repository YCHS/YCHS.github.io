...
function loadElements(){
		...
		for (var i = 0; i < zodiacData.length; i++){
				...
				link.className = "slide-nav";
				link.id = i + "-link";
				link.innerHTML = zodiacData[i].name;
				if (i == 0){
				...
				// Make each zodiac 
				var container = makeElement("div", "z-container z-" + zodiacData[i].name.toLowerCase());
				container.id = i + "-zodiac";

				if (i == 0) {
						container.classList.add("z-active"); 
				}
				else {
						container.classList.add("animate-start"); 
				}

				var itemLeft = makeElement("div", "z-item z-item-left");
				var content = makeElement("div", "z-content");
				var title = makeElement("h1", "text-big"); 
				title.innerHTML = zodiacData[i].name;
				
				var text = makeElement("p", "text-normal");
				text.innerHTML = zodiacData[i].text;

				content.appendChild(title);
				content.appendChild(text);
				itemLeft.appendChild(content);

				var itemRight = makeElement("div", "z-item z-item-right");
				var img = makeElement("img", "z-image");
